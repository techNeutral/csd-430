//Brett Fuller
//CSD-430 Assignment CRUD Project
//7/6/2025
...
public class MyMovieBean implements java.io.Serializable{
	...
	//Module 8
	//Code for updating a record. This method takes information for all relevant fields finds the record using the primary key, year, and then updates the record.
    public String updateRecord(String title, String director, int year, 
            String leadActor, String leadActress) {
    	
		String sql = "UPDATE fuller_movies_data SET Title = ?, Director = ?,  LeadActor = ?, LeadActress = ? WHERE Year = ?";

		try {
	    		java.sql.PreparedStatement sqlStatement = connection.prepareStatement(sql);
	    		
	    		sqlStatement.setString(1, title );    		
	    		sqlStatement.setString(2, director );    		
	    		sqlStatement.setString(3, leadActor );    		
	    		sqlStatement.setString(4, leadActress );
	...
	//Module 8
    //Code for entering information to be used for updating a record. 
	//This creates a form and populates it with the data from the database. All fields except for the year allow for free text information.
    public String formUpdate(String requestURL) {
    	...
    	try{
        	...
        	resultSet = statement.executeQuery("SELECT DISTINCT Title FROM fuller_movies_data ORDER BY Title ASC");
        }
    	...
    	//Title
       //Form info for the title queries the database for all titles and creates a drop down. 
    	//Can select data from that field use it, edit it, or enter your own information
    	
    	dataStringBuilder.append("<label for='title'>Movie Title</label>\n");
    	dataStringBuilder.append("&nbsp&nbsp \n");  
    	dataStringBuilder.append("<input list='title' name='title'>\n");      	
    	...
    	//Director
        //Form info for the Director queries the database for all titles and creates a drop down. 
     	//Can select data from that field use it, edit it, or enter your own information

    	try{
    		
        	resultSet = statement.executeQuery("SELECT DISTINCT Director FROM fuller_movies_data ORDER BY Director ASC");
    	...
    	dataStringBuilder.append("&nbsp&nbsp \n");   
    	dataStringBuilder.append("<input list='director' name='director'>\n");      	
    	...
    	//LeadActor
        //Form info for the LeadActor queries the database for all titles and creates a drop down. 
     	//Can select data from that field use it, edit it, or enter your own information
    	try{
    		
        	resultSet = statement.executeQuery("SELECT DISTINCT LeadActor FROM fuller_movies_data ORDER BY LeadActor ASC");
    	...
    	dataStringBuilder.append("<label for='leadActor'>Lead Actor</label>\n");
    	dataStringBuilder.append("&nbsp&nbsp \n");  
    	dataStringBuilder.append("<input list='leadActor' name='leadActor'>\n");      	
    	dataStringBuilder.append("<datalist id='leadActor'>\n");   
    	...
    	//LeadActress
        //Form info for the LeadActress queries the database for all titles and creates a drop down. 
     	//Can select data from that field use it, edit it, or enter your own information

    	try{
    		
        	resultSet = statement.executeQuery("SELECT DISTINCT LeadActress FROM fuller_movies_data ORDER BY LeadActress ASC");
    	...
    	dataStringBuilder.append("<label for='leadActress'>Lead Actress</label>\n");
    	dataStringBuilder.append("&nbsp&nbsp \n");   
    	dataStringBuilder.append("<input list='leadActress' name='leadActress'>\n");      	
    	dataStringBuilder.append("<datalist id='leadActress'>\n");  
    	...
    	//Year
        //Form info for the Year queries the database for all titles and creates a drop down this field cannot be edited, only records in the list can be selected.
    	try{
    		
    		resultSet = statement.executeQuery("SELECT DISTINCT Year FROM fuller_movies_data ORDER BY Year ASC");
        }
        catch(java.sql.SQLException e){
	...
	public String createRecord(String Title, String Director, int Year, String LeadActor, String LeadActress) {
		StringBuilder message = new StringBuilder();
		boolean isAllowed = true;
		//Validate if year greater than 0 and that the year is not already in the table and set boolean and message accordingly
		if(Year > 0){  	
		    ...
		    try {
		    	if(resultSet.next()) {
		    		message.append("Error:  The Year " + Year + " already has a record");
			    	isAllowed = false;
		    	}
		    }
		    catch ( java.sql.SQLException e) {
		...
		//validate that there is a value for the string values
		if(Title.equals("") || Director.equals("") || LeadActor.equals("") || LeadActress.equals("")) {
			isAllowed = false;
			message.append("Error:  All fields require values");
		}
		
		//add record if allowed
		if(isAllowed) {
			try {

	    		String sql = "INSERT INTO fuller_movies_data(Title, Director, Year, LeadActor, LeadActress)" + 
	    		   "VALUES(?, ?, ?, ?, ?)";
	    		//"INSERT INTO fuller_movies_data(Title, Director, Year, LeadActor, LeadActress)VALUES('Anora', 'Sean Baker', 2024, 'Mark Eydelshteyn', 'Mikey Madison')"
	    		...
	    		sqlStatement.setString(4, LeadActor );    		
	    		sqlStatement.setString(5, LeadActress );
